...
const main = async () => {
  const { data, error } = await supabase.from('products').select('id,mrp,sku_l,sku_w,sku_h,sku_dim_unit,sku_nw,sku_gw,sku_wt_unit');
  ...
  for (const p of data || []) {
    ...
    const r = await supabase.from('showroom_items').update({mrp:p.mrp,sku_l:p.sku_l,sku_w:p.sku_w,sku_h:p.sku_h,sku_dim_unit:p.sku_dim_unit,sku_nw:p.sku_nw,sku_gw:p.sku_gw,sku_wt_unit:p.sku_wt_unit,dimensions}).eq('source_type','product').eq('source_id',p.id);
