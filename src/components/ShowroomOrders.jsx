...
export default function ShowroomOrders({ canManageQuotations = false }) {
  ...
  async function updateItem(orderId, itemId, field, value) {
    ...
    if (err) setError(err.message); else patchLocal(orderId, itemId, patch);
    setSaving(null);
  }
      ...
      {filtered.map(order => {
        ...
        return <section className="panel glass-panel showroom-order-card" key={order.id}>
          ...
          <div className="showroom-order-table">
            <div className="showroom-order-table-head"><span>Product</span><span>Qty</span><span>Required</span><span>Availability</span><span>Unit price</span><span>Line total</span><span>Accounts note</span></div>
