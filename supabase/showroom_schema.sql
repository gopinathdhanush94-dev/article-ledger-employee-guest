...
insert into public.showroom_items (
  source_type, source_id, ean, article_no, mrp, name, brand, model, category,
  description, image_url, features, dimensions,
  sku_l, sku_w, sku_h, sku_dim_unit, sku_nw, sku_gw, sku_wt_unit,
  featured, visible
)
select
  'product', p.id, p.ean, p.article_no, p.mrp,
...
-- Guest showroom order requests. Only authenticated guest users can create orders;
-- active employees/admins can review them later from the employee application.
...
create policy "Guests can create showroom orders"
on public.showroom_order_requests for insert to authenticated
with check (created_by = auth.uid() and public.current_account_type() = 'guest' and public.current_app_role() = 'guest');

drop policy if exists "Guests can create showroom order items" on public.showroom_order_items;
create policy "Guests can create showroom order items"
on public.showroom_order_items for insert to authenticated
with check (exists (select 1 from public.showroom_order_requests o where o.id = order_id and o.created_by = auth.uid()));

drop policy if exists "Guests can delete their showroom orders" on public.showroom_order_requests;
create policy "Guests can delete their showroom orders"
on public.showroom_order_requests for delete to authenticated
...
create policy "Guests can view their showroom orders"
on public.showroom_order_requests for select to authenticated
using (created_by = auth.uid());

drop policy if exists "Guests can view their showroom order items" on public.showroom_order_items;
create policy "Guests can view their showroom order items"
on public.showroom_order_items for select to authenticated
using (exists (select 1 from public.showroom_order_requests o where o.id = order_id and o.created_by = auth.uid()));

drop policy if exists "Employees can view showroom orders" on public.showroom_order_requests;
create policy "Employees can view showroom orders"
on public.showroom_order_requests for select to authenticated
using (public.is_active_employee());

drop policy if exists "Employees can view showroom order items" on public.showroom_order_items;
create policy "Employees can view showroom order items"
on public.showroom_order_items for select to authenticated
using (public.is_active_employee());
...
create trigger trg_showroom_order_requests_updated_at
before update on public.showroom_order_requests
